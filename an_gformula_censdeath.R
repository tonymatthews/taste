...
for(i in outcome_sens) {
  ...
  death <- dat_death %>% 
    select(lopnr, date_event) %>% 
    rename(date_death=date_event)
  
  dat <- dat %>% 
    merge(death, by="lopnr") 
  
  #Expand to a row per week until event, and censor at death
  dat_model <- dat %>%
    mutate(endoffup = pmin(date_event, date_death, na.rm=TRUE)) %>% 
    mutate(survtime = ceiling((endoffup-interdat)/7)) %>% 
    mutate(survtime = replace(survtime, survtime>52, 52)) %>% 
    mutate(survtime = replace(survtime, is.na(survtime), 52)) %>%
...
write.table(results_1y, paste0("logfiles/an_gformula_censdeath_", period, ".txt"), sep="\t", quote=FALSE, row.names=FALSE)
