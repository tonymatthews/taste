...
results_3y <- data.frame()
difference_results_3y <- data.frame()
...
for(i in outcomes) {
  ...
  dat_model <- dat %>%
    mutate(survtime = ifelse(event_3y==0 | (date_event-interdat)/7>156 , 156,
                             ceiling((date_event-interdat)/7)))  %>%
    expandRows("survtime", drop=F) %>%
    mutate(time = sequence(rle(lopnr)$lengths)-1) %>% 
    mutate(event = ifelse(time==survtime-1 & event_3y==1, 1, 0)) %>%
    mutate(timesq = time^2)  %>%
    select(-event_30, -event_1y, -event_3y, -date_event, -interdat, -survtime)
  ...
  for(m in 1:length(confounder_list)) {
    ...
    gf_exp0 <- expandRows(dat, count=156, count.is.col=F)
    gf_exp0$time <- rep(seq(0,155), nrow(dat))
    ...
    surv_0 <- gf$surv_0[156]
    surv_1 <- gf$surv_1[156]
    Y_0 <- 1-surv_0
    Y_1 <- 1-surv_1
    riskdiff <- Y_1 - Y_0
    riskratio <- Y_1/Y_0
  ...
  for (z in 1:numboot) {
    ...
    for(m in 1:length(confounder_list)) {
      ...
      #create dataset with all time points for each individual under each treatment
      gf_exp0 <- expandRows(boot_dat, count=156, count.is.col=F)
      gf_exp0$time <- rep(seq(0,155), nrow(dat))
      gf_exp0$timesq <- gf_exp0$time^2
      gf_exp0$exp <- 0
  ...
  results_3y <- rbind(results_3y, all_results_full)
  ...
  difference_results_3y <- rbind(difference_results_3y, difference_results_full)
  
  
  # PLOT STANDARDIZED CURVES FOR FULLY ADJUSTED MODEL ONLY - FINAL ONE
